...
def handle_cors_headers(handler, status=200):
    ...
    matched_origin = None
    if origin and (origin.rstrip("/") in allowed_origins or origin.startswith("http://localhost:") or origin.startswith("http://127.0.0.1:")):
        matched_origin = origin
    elif not origin:
        # Same-origin request
        matched_origin = "http://localhost:8000"
